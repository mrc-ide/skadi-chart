...
import { AllOptionalLayers, Bounds, D3Selection, LayerArgs, ZoomExtents, PartialScales, Point, Scales, ScatterPoints, XY, XYLabel, ScaleNumeric, AxisType, UserLines } from "./types";
...
export class Chart<Metadata = any> {
  ...
  private filterLinesForLogAxis = (lines: UserLines<Metadata>, axis: "x" | "y") => {
    let warningMsg = "";
    const filteredLines: UserLines<Metadata> = [];
    for (let i = 0; i < lines.length; i++) {
      const currLine = lines[i];
      let isLastCoordinatePositive = currLine.points[0] && currLine.points[0][axis] > 0;
      let lineSegment: UserLines<Metadata>[number] = { ...currLine, points: [] };
  ...
  private filterLines = (lines: UserLines<Metadata>) => {
  ...
  addTraces = (lines: UserLines<Metadata>, options?: Partial<TracesOptions>) => {
