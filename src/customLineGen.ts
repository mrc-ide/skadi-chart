...
const pointIsInRange = (p: Point, zoomExtents: ZoomExtents) => {
  return zoomExtents.x[0] <= p.x && p.x <= zoomExtents.x[1] && zoomExtents.y[1] <= p.y && p.y <= zoomExtents.y[0];
}
...
export const customLineGen = (lineSC: Point[], zoomExtents: ZoomExtents, fillArea?: boolean) => {
  ...
  let wasLastPointInRange = pointIsInRange(lineSC[0], zoomExtents);
  ...
  for (let i = 0; i < lineSC.length; i++) {
    ...
    if (wasLastPointInRange) {
      ...
      const prevPoint = getNewSvgPoint(lineSC[i - 1], "M");
      currentSegmentPath += prevPoint;
      currentSegmentPath += getNewSvgPoint(lineSC[i], "L");
    }
